...
jQuery(function($){
  ...
  jQuery(document).on('click', 'input.select-img', function( event ){
    ...
    var AscendoorBlogImage = wp.media.controller.Library.extend({
      defaults :  _.defaults({
    ...
    ascendoor_blog_file_frame = wp.media.frames.ascendoor_blog_file_frame = wp.media({
      button: {
        text: jQuery(this).data('uploader_button_text'),
      },
      state: 'author-insert-image',
      states: [
        new AscendoorBlogImage()
        ],
            multiple: false  // Set to true to allow multiple files to be selected
          });

        // When an image is selected, run a callback.
    ascendoor_blog_file_frame.on('select', function() {
      var state = ascendoor_blog_file_frame.state('author-insert-image');
    ...
    ascendoor_blog_file_frame.open();
  });
