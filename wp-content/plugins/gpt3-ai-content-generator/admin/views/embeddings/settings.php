...
<div id="wpaicg_gcpStarterModal" style="display:none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 9999;">
    <div style="background: #fff; width: 400px; padding: 20px; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);">
        <p><?php echo sprintf(esc_html__('You have selected the GCP Starter environment. Please note that this environment has limitations. Check the limitations %shere%s and %shere%s.', 'gpt3-ai-content-generator'), '<a href="https://docs.pinecone.io/docs/starter-environment" target="_blank">', '</a>', '<a href="https://docs.aipower.org/docs/ChatGPT/advanced-setup/pdfchat#limitations" target="_blank">', '</a>'); ?></p>
        <button id="wpaicg_closeModal"><?php echo esc_html__('Close', 'gpt3-ai-content-generator'); ?></button>
    </div>
</div>
<script>
    jQuery(document).ready(function($){
        // Display the modal when GCP Starter is selected
        $('.wpaicg_pinecone_sv').change(function() {
        if ($(this).val() === 'gcp-starter') {
            $('#wpaicg_gcpStarterModal').fadeIn();
        }
        });

        // Close the modal when the close button is clicked
        $('#wpaicg_closeModal').click(function() {
            $('#wpaicg_gcpStarterModal').fadeOut();
        });
                    ...
                    success: function (res){
                        if(res.length){
                            ...
                            for(var i=0; i<res.length; i++){
                                (function (indexName) { // Using an IIFE to capture the current indexName value
                                    $.ajax({
                                        ...
                                        success: function(resi){
                                            ...
                                            if(selectedLists.length === res.length){ // Check if all AJAX calls have completed
                                ...
                                })(res[i]); // Pass the current indexName value to the IIFE
                            }
                        }
                    },
                    error: function (e){
