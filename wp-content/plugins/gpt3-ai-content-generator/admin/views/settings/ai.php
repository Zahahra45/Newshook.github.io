...
<div id="tabs-1">
    ...
    <select class="regular-text" id="wpaicg_ai_model" name="wpaicg_ai_model">
        ...
        // Check and correct the format of $custom_models to be associative if necessary
        if (is_array($custom_models)) {
            $formatted_custom_models = [];
            foreach ($custom_models as $index => $model) {
                $formatted_custom_models[$model] = $model; // Use model string as both key and value
            }
        ...
        <optgroup label="GPT-3">
            <?php foreach ($gpt3_models as $value => $name): ?>
                <option value="<?php echo esc_attr($value); ?>"<?php selected($value, $current_model); ?>><?php echo esc_html($name); ?></option>
            <?php endforeach; ?>
        </optgroup>
        <optgroup label="Legacy Models">
            <?php foreach ($legacy_models as $value => $name): ?>
                <option value="<?php echo esc_attr($value); ?>"<?php selected($value, $current_model); ?>><?php echo esc_html($name); ?></option>
            <?php endforeach; ?>
        </optgroup>
