...
if ( isset( $_POST['wpaicg_submit'] ) ) {
    ...
    if (isset($_POST['wpaicg_azure_api_key'])) {
        update_option('wpaicg_azure_api_key', sanitize_text_field($_POST['wpaicg_azure_api_key']));
    }

    if (isset($_POST['wpaicg_azure_endpoint'])) {
        update_option('wpaicg_azure_endpoint', sanitize_text_field($_POST['wpaicg_azure_endpoint']));
    }

    if (isset($_POST['wpaicg_azure_deployment'])) {
        update_option('wpaicg_azure_deployment', sanitize_text_field($_POST['wpaicg_azure_deployment']));
    }

    if (isset($_POST['wpaicg_azure_embeddings'])) {
        update_option('wpaicg_azure_embeddings', sanitize_text_field($_POST['wpaicg_azure_embeddings']));
    }
    ...
    if (isset($_POST['wpaicg_provider'])) {
        ...
        if ($_POST['wpaicg_provider'] === 'Azure') {
    ...
    $data = [
        ...
        'wpai_add_img'           => ( !isset( $wpai_add_img ) || $wpai_add_img ? 1 : 0 ),
    ...
    if ( $flag == true ) {
        ...
        $wpaicg_keys = array(
            ...
            'wpaicg_woo_custom_prompt_focus_keyword',
        ...
        foreach($wpaicg_keys as $wpaicg_key){
            if(isset($_POST[$wpaicg_key]) && !empty($_POST[$wpaicg_key])){
                if($wpaicg_key == 'wpaicg_content_custom_prompt'){
                    ...
                    $sanitized_data = \WPAICG\wpaicg_util_core()->sanitize_text_or_array_field($_POST[$wpaicg_key]);
